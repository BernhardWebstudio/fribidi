    ...
    const FriBidiStrIndex len)
{
  ...
  for (i = 0; i < len; i++)
  {
    ...
    if (char_type != last->type || bracket_type != FRIBIDI_NO_BRACKET /* Always separate bracket into single char runs! */
        || last->bracket_type != FRIBIDI_NO_BRACKET || FRIBIDI_IS_ISOLATE(char_type))
    {
      run = new_run();
      if
        UNLIKELY(!run)
